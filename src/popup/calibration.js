async function handleCalibrationUpload(file) {
  try {
      ...
      if (residuals) {
        ...
        //   `Tracking active using ${config.coordinateSystem.toUpperCase()} mode with ${config.landmarkPoints}-point tracking (RMSE: ${residuals.rmse.toFixed(2)} px)`;
        ...
        // document.getElementById("status").textContent = 
        //   `Tracking active using ${config.coordinateSystem.toUpperCase()} mode with ${config.landmarkPoints}-point tracking`;
      }
